...
function parseMonth01() {
  ...
  // Part B: January Salaries (26년 1월 레저사업본부 급여등.xlsx, Sheet: 레저)
  const salFile = `${GDRIVE_DIR}/26년 1월 레저사업본부 급여등.xlsx`;
  const wbSal = XLSX.readFile(salFile);
  const dataSal = XLSX.utils.sheet_to_json(wbSal.Sheets['레저'], { header: 1 });

  // Rows 2 through 7: 모토아레나팀, Activity팀, 디지털지원팀, 레저사업본부, 목장팀, 미디어아트팀
  for (let r = 2; r <= 7; r++) {
    const row = dataSal[r];
    if (!row) continue;
    const rawTeam = String(row[1] || '').trim();
    const basePay = Number(row[2] || 0);     // 급여
    const partTime = Number(row[3] || 0);    // 잡급
    const insurance = Number(row[4] || 0);   // 4대보험료 소계
    const retirement = Number(row[5] || 0);  // 퇴직급여
    const meal = Number(row[6] || 0);        // 식대

    let assignedTeam = '본부공통';
    let assignedVenue = '레져본부 (공통)';
    if (rawTeam.includes('Activity')) { assignedTeam = '액티비티'; assignedVenue = '액티비티 (공통)'; }
    else if (rawTeam.includes('디지털')) { assignedTeam = '디지털지원'; assignedVenue = '디지털지원팀'; }
    else if (rawTeam.includes('목장')) { assignedTeam = '목장'; assignedVenue = '벨포레 목장'; }
    else if (rawTeam.includes('미디어')) { assignedTeam = '미디어아트센터'; assignedVenue = '미디어아트센터'; }
    else if (rawTeam.includes('모토')) { assignedTeam = '외주'; assignedVenue = '모토아레나 (별도본부)'; }
    ...
    if (basePay > 0) {
      parsedRows.push({
        ...
        accountName: '급여',
        macroCategory: '인건비',
        rawDepartment: rawTeam,
        amount: basePay,
        memo: `1월귀속 직원급여 / ${rawTeam}`,
        clientName: '정규직원',
        assignedTeam,
        assignedVenue,
        assignedCategory: '인건비',
    ...
    if (partTime > 0) {
      parsedRows.push({
        ...
        rawDepartment: rawTeam,
        amount: partTime,
        memo: `1월귀속 일용직/아르바이트 노임 / ${rawTeam}`,
        clientName: '일용노임',
        assignedTeam,
        assignedVenue,
        assignedCategory: '인건비',
        friendlyCategory: '아르바이트비 (알바비)',
      });
    }
    ...
    if (insurance > 0) {
      parsedRows.push({
        ...
        rawDepartment: rawTeam,
        amount: insurance,
        memo: `1월귀속 4대보험료 및 국민연금 / ${rawTeam}`,
        clientName: '국민건강보험공단',
        assignedTeam,
        assignedVenue,
        assignedCategory: '복리후생비',
    ...
    if (meal > 0) {
      parsedRows.push({
        ...
        rawDepartment: rawTeam,
        amount: meal,
        memo: `1월귀속 직원 식대 / ${rawTeam}`,
        clientName: '직원식대',
        assignedTeam,
        assignedVenue,
        assignedCategory: '복리후생비',
        friendlyCategory: '직원 밥값과 간식비',
      });
    }
  }
