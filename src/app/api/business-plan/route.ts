...
export async function GET(request: Request) {
  try {
    ...
    let url = `${BACKEND_URL}/api/v6/report/business-plan?year=${year}`;
    
    const res = await fetch(url, { headers: m2mHeaders, cache: 'no-store' });
    if (!res.ok) {
      console.error(`Failed to fetch API v6 business-plan: HTTP ${res.status}`);
    ...
    console.error('Business Plan Data Fetch Error:', error);
