...
export async function GET(request: Request) {
  try {
    ...
    let url = `${BACKEND_URL}/api/v6/report/team-report`;
    ...
    if (!res.ok) {
      console.error(`Failed to fetch API v6 team-report: HTTP ${res.status}`);
    ...
    console.error('Team Report Data Fetch Error:', error);
