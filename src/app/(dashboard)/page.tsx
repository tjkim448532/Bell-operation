...
export default function Dashboard() {
  ...
  // V5 API 실시간 데이터 우선 적용 (목표 시트 데이터가 없으면 백엔드 숙박객 수로 대체)
  const displayTotalVisitors = totalVisitorActual > 0 ? totalVisitorActual : (data?.preCalculatedExpectedGuests || 0);
        ...
        <div className="bg-gradient-to-br from-[#0c3c2e] to-[#156e54] rounded-3xl shadow-lg p-6 text-white relative overflow-hidden flex flex-col justify-center min-h-[140px]">
          ...
          <div className="relative z-10 flex items-center gap-4">
            ...
            <div>
              ...
              <h2 className="text-3xl font-extrabold mt-1">{displayTotalVisitors.toLocaleString()} <span className="text-xl font-bold">명</span></h2>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          ...
          <div className="space-y-6">
            {dynamicTeams.length > 0 ? dynamicTeams.map((team) => {
              ...
              const exactTeamVisitors = data?.leisureTeamVisitors?.[team] || 0;
              
              // 실시간 백엔드 기반 이용률 계산 (V5 API)
              const actualRate = expectedRoomGuests > 0 ? (exactTeamVisitors / expectedRoomGuests) * 100 : 0;
              ...
              const goalRate = count > 0 ? sumGoal / count : 0;

              return (
              <div key={team} className="group">
                <div className="flex justify-between items-end mb-3">
                  <span className="font-bold text-gray-800 text-lg">{team}</span>
                  <div className="text-right flex flex-col items-end gap-1.5">
                    <div className="text-sm flex items-center">
                      <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-md mr-2 font-medium">실제 / 목표</span>
                      <span className="font-bold text-gray-900 text-base">{actualRate > 0 ? `${actualRate.toFixed(1)}%` : '0%'}</span>
                      <span className="text-gray-400 ml-1 text-xs">/ {goalRate > 0 ? `${goalRate.toFixed(1)}%` : 'N/A'}</span>
                    </div>
                ...
                <div className="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden relative">
                  <div 
                    ...
                    style={{ width: `${goalRate}%`, opacity: 0.5 }}
                  />
                  <div 
                    className={`absolute top-0 left-0 h-full rounded-full transition-all ${actualRate >= goalRate && goalRate > 0 ? 'bg-emerald-600' : 'bg-emerald-400'}`}
              ...
              <p className="text-gray-500 text-center py-8">목표 시트 연동 설정이 없습니다.</p>
            )}
          </div>
        </div>
